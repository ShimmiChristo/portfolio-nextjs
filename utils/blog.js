...
const postsDirectory = path.join(process.cwd(), 'blog'); // process.cwd() returns the absolute path of the current working directory
...
export function getSortedPostsData() {
  ...
  const allPostsData = fileIds.map((fileId) => {
    const fileName = fs.readdirSync(path.join(postsDirectory, `${fileId}`));

    // Remove ".md" from file name to get id
    const name = fileName[0].replace(/\.md$/, ''); // id = 'pre-rendering', 'ssg-ssr'

    // Read markdown file as string
    const fullPath = path.join(postsDirectory, fileId, fileName[0]);
...
export async function getPostData(id, postname) {
  const fullPath = path.join(postsDirectory, `${id}`, `${postname}.md`);
